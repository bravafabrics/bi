...
$matrix_absolutios = fulfill_valores_matrix($current_month);
$matrix_relativos = fulfill_relativos_matrix($current_month);
$average = average_from_relativos_matrix($matrix_relativos, $current_month);
$average_first_half = array_slice($average, 0, count($average) / 2);
	echo "<div style=\"text-align:center;\"><h3>Last Recurrency (long term) :</h3> <h1>".array_sum($average)."</h1><br />
	<h3>Last Recurrency (short term) :</h3> <h1>".array_sum($average_first_half)."</h1></div><br /><br />";
echo "<h1>Valores absolutos = número de pedidos</h1>";
see_a_matrix_in_a_table($matrix_absolutios);
echo "<br />";
echo "<h1>Valores relativos (pedido / cliente de la cohorte)</h1><br />";
see_a_matrix_in_a_table($matrix_relativos);


see_array_and_return_recurrency($average);
