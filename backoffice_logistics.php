...
try{	
$db_conn = mysqli_connect(DBBV_HOST, DBBV_USER, DBBV_PASS, DBBV_DATABASE);
...
try {
	$db_conn = null;
	$db_conn = new PDO('mysql:host='.DBBV_HOST.';dbname='.DBBV_DATABASE.';charset=utf8', DBBV_USER, DBBV_PASS); 
...
function return_the_logistic_final() { 
...
function report_from_exchanges_and_refunds_about_devoluciones($date) {  

// Remove < by = after the first fulfilling.

$query = "SELECT id AS 'id_refund', id_order, return_date, country, refunded_novat, pay_method,
CASE
...
1
END
 as 'logistic_consideration'
 FROM devoluciones WHERE return_date = '".$date."' ";
...
function report_from_exchanges_about_envios($date) {  


$query = "
SELECT country, DATE_FORMAT(created_at,'%Y-%m-%d') AS 'date', COUNT(*) AS 'number_returns',
				(
				CASE
				when country='ES' OR country='PT' THEN
				'".Returns_go_es."'
...
FROM envios  WHERE DATE_FORMAT(created_at,'%Y-%m-%d') = '".$date."' AND type = 'Cambio' GROUP BY country
...
// $result = return_the_logistic_final();
